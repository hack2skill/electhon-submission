...
import AddPollForm from "../utils/AddPollForm";
import { DataContext } from "../context/Context";
import { NavLink } from "react-router-dom";
const Navbar = () => {
  const { setFormModal, formModal, address, getElectionCommissionHead } =
    useContext(DataContext);

  return (
    <>
      <header className="header ">
        <div className="header__wrapper px-10">
          <a className="brand-logo mt-2 font-bold" href="#">
            <NavLink to="/">
              <h1 className="mt-1 text-2xl font-extrabold text-gray-900 dark:text-black md:text-2xl lg:text-2xl">
          ...
          <div className="nav-bar flex  mx-3">
            <div className="flex">
              {address ? (
                <>
                  {address === getElectionCommissionHead() && (
                    <button
                      type="button"
                      onClick={() => {
                      ...
                      className="mt-2 mx-2 text-gray-900 bg-white  focus:outline-none hover:bg-gray-100  focus:ring-gray-200 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-gray-800 dark:text-white dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700"
                    >
                      Add Poll
                    </button>
                  )}
                  <button
                    ...
                    className="mt-2 mx-2 text-gray-900 bg-white  focus:outline-none hover:bg-gray-100  focus:ring-gray-200 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-gray-800 dark:text-white dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700"
                  >
                    <NavLink to="/election-polls">Elections listed</NavLink>
                  </button>
                </>
              ) : null}
              <span className="mt-2">
                ...
                {/* <button
                  ...
                  className="flex items-center mx-3 p-2 text-base font-bold text-gray-900 rounded-lg bg-gray-50 hover:bg-gray-100 group hover:shadow dark:bg-gray-600 dark:hover:bg-gray-500 dark:text-white"
                >
                  <svg
                    aria-hidden="true"
                    class="h-4"
                  ...
                  <span class="flex-1 ml-3 whitespace-nowrap">{address ? address.toString().slice(0,10): "MetaMask"}</span>
              ...
              <NavLink to="/dashboard" className="mt-2 mx-2">
                <img
